...
export function CropImageNode({ id, data, selected }: NodeProps) {
    ...
    const pollForCompletion = async (runId: string): Promise<string> => {
        ...
        for (let i = 0; i < 300; i++) {
            ...
            if (["FAILED", "CANCELED", "CRASHED"].includes(result.status)) {
                ...
                // @ts-ignore
                const rawError = result.error?.message || result.error || result.status;
    ...
    const runCrop = async () => {
        ...
        toast.info("Starting crop job..."); // ✅ Added for better UX
        ...
        try {
            const finalUrl = await logExecution(
                ...
                async () => {
                    ...
                    console.log("Crop Job Started:", res.runId);
            ...
            // ✅ FIX: Safer message extraction
            toast.error(err.message || "Crop failed. Check logs.");
        } finally {
